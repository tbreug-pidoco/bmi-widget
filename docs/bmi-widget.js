...
BMI should not be used as the sole measure of a person's health, but in combination with other methods and measurements.`,bmi:"Your BMI",errorAge:"Please enter your age in years as a number between 19 and 120!",errorSize:"Please enter your height in centimetres as a number between 50 and 250!",errorWeight:"Please enter your weight in kilograms as a number between 20 and 350!",young:`This BMI Calculator is suitable for adults from 19 years old.
Depending on their age very different values are considered to be normal for adolescents.
Weight alone is not a decisive factor for health status. Your doctor will evaluate the weight of your child best and thereby take into account his/her personal development.`,underweight:"This value is <b>below the average mark</b> of [%BMI_FROM%-%BMI_TO%]. There can be various reasons for this. When in doubt please consult your doctor!",normalweight:"Very good! This value is <b>within the average range</b> of [%BMI_FROM%-%BMI_TO%] for your age class. You clearly value nutrition and exercise. Keep at it!",overweight:"Not quite perfect. This value is <b>slightly above the average mark</b> of [%BMI_FROM%-%BMI_TO%] for your age class. You can reduce your weight with regular exercise as well as a healthy and balanced diet.",obese:"This value is <b>well above the average mark</b> of [%BMI_FROM%-%BMI_TO%] for your age class. Your weight may pose a risk to your health. We recommend you consult your doctor for further information."}},r=(o,t)=>(u[t]||u.de)[o]||u.de[o],e="bmi-widget",l=(o,t)=>{const i=document.createElement(o);return t&&(i.className=t),i},f=(o,t,i,a,s,n)=>{const d=l("label");return d.innerHTML=`<input type="radio" id="${t}" name="${i}" value="${a}" ${s?"checked":""}/>${n}`,o.appendChild(d),d.getElementsByTagName("input")[0]};class m{constructor(t,i,a,s){h(this,"unit");h(this,"listener");h(this,"element");this.unit=a,this.listener=s;const n=l("div",`${e}_value`),d=l("label");d.innerText=i,n.appendChild(d),this.element=l("input"),this.element.type="text",this.element.inputMode="numeric",this.element.placeholder=a,this.element.title=i+" ("+a+")",this.element.onkeydown=c=>{c.keyCode>=65&&c.preventDefault()},this.element.onfocus=()=>this.units=!1,this.element.onblur=()=>this.units=!0,this.element.oninput=()=>this.listener.changed(this),n.appendChild(this.element),t.appendChild(n)}get parse(){return parseInt(this.element.value)}get valid(){const t=this.parse;return!isNaN(t)&&t>0}set valid(t){t?this.element.classList.remove("invalid"):(this.element.classList.add("invalid"),this.element.focus(),this.units=!1,this.element.setSelectionRange(0,-1))}set units(t){const i=this.parse;this.element.value=isNaN(i)?"":i.toString()+(t?" "+this.unit:"")}}const w=`
	.${e} * {
